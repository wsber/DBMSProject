...
private:
    RC  InsertFree   (int slot);                 // 在空闲块链头插入槽号为slot的空闲块（实质为变更PF_BufPageDesc结构体内部指针值）
    RC  LinkHead     (int slot);                 // 在使用块链头插入槽号为slot的缓冲块
    RC  Unlink       (int slot);                 // 释放槽号为slot的被使用的缓冲块，将其加入空闲块链
    RC  InternalAlloc(int &slot);                // 得到一个槽号slot去使用
